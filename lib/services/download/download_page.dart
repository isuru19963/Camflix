...
import 'package:nexthour/models/progress_data.dart';
import 'package:nexthour/models/task_info.dart';
import 'package:nexthour/models/todo.dart';
import 'package:nexthour/models/user_profile_model.dart';
import 'package:nexthour/providers/user_profile_provider.dart';
import 'package:nexthour/services/player/downloaded_video_player.dart';
import 'package:nexthour/services/repository/database_creator.dart';
import 'package:nexthour/services/repository/repository_service_todo.dart';
...
class _DownloadPageState extends State<DownloadPage> with TickerProviderStateMixin {
  ...
  Future<Null> _prepare() async {
    ...
    dTasks.add(
      TaskInfo(
        ...
        ifLink: widget.videoDetail.videoLink == null ? null : "${widget.videoDetail.videoLink.iframeurl}",
        hdLink: widget.videoDetail.videoLink == null ? null : "${widget.videoDetail.videoLink.readyUrl}",
        link360: widget.videoDetail.videoLink == null ? null : "${widget.videoDetail.videoLink.url360}",
        link480: widget.videoDetail.videoLink == null ? null : "${widget.videoDetail.videoLink.url480}",
        link720: widget.videoDetail.videoLink == null ? null : "${widget.videoDetail.videoLink.url720}",
        link1080: widget.videoDetail.videoLink == null ? null : "${widget.videoDetail.videoLink.url1080}",
      ),
    );
  ...
  void _showDialog(task) {
    var userProfileProvider = Provider.of<UserProfileProvider>(context, listen: false).userProfileModel;
    getAllScreens();
    var downCount;
            ...
            content: Container(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  widget.videoDetail.videoLink.url360 == "null" ? SizedBox.shrink() : Padding(
                  ...
                  widget.videoDetail.videoLink.url480 == "null" ? SizedBox.shrink() : Padding(
                    padding: EdgeInsets.only(left: 50.0, right: 50.0),
                    child: RaisedButton(
                      color: activeDotColor,
                      hoverColor: Colors.red,
                  ...
                  widget.videoDetail.videoLink.url720 == "null" ? SizedBox.shrink() : Padding(
                    padding: EdgeInsets.only(left: 50.0, right: 50.0),
                    child: RaisedButton(
                      hoverColor: Colors.red,
                      splashColor: Color.fromRGBO(49, 131, 41, 1.0),
                  ...
                  widget.videoDetail.videoLink.url1080 == "null" ? SizedBox.shrink() : Padding(
                    padding: EdgeInsets.only(left: 50.0, right: 50.0),
                    child: RaisedButton(
                      hoverColor: Colors.red,
                      splashColor: Color.fromRGBO(49, 131, 41, 1.0),
  ...
  Widget _buildActionForTask(TaskInfo task) {
    ...
    if (task.status == DownloadTaskStatus.undefined) {
      ...
      createTodo(task, task.name, dFileName, mVType,
          widget.videoDetail.id, task.taskId, progress, task.hdLink);
      return Row(
        mainAxisSize: MainAxisSize.min,
  ...
  void _delete(TaskInfo task) async {
    ...
    var raw = await cdb.delete(
      ...
      where: "movie_id = ? AND vtype = ?", whereArgs: ['${widget.videoDetail.id}', '$mVType'],
    );
    setState(() {});
  }
  ...
  Future<Todo> getPersonWithId(
    ...
    var userDetails = Provider.of<UserProfileProvider>(context, listen: false).userProfileModel.user;
  ...
  void _requestDownload(TaskInfo task) async {
    ...
    int progress = 0;
    var mVType = widget.videoDetail.type == DatumType.T ? "T" : "M";
    createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id, task.taskId, progress, task.hdLink);
  }
  ...
  void _requestDownload360(TaskInfo task) async {
    ...
    int progress = 0;
    var mVType = widget.videoDetail.type == DatumType.T ? "T" : "M";
    createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id, task.taskId, progress, task.link360);
  }
  void _requestDownload480(TaskInfo task) async {
    ...
    int progress = 0;
    var mVType = widget.videoDetail.type == DatumType.T ? "T" : "M";
    createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id, task.taskId, progress, task.link480);
  }
  void _requestDownload720(TaskInfo task) async {
    ...
    int progress = 0;
    var mVType = widget.videoDetail.type == DatumType.T ? "T" : "M";
    createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id, task.taskId, progress, task.link720);
  }
  void _requestDownload1080(TaskInfo task) async {
    ...
    int progress = 0;
    var mVType = widget.videoDetail.type == DatumType.T ? "T" : "M";
    createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id, task.taskId, progress, task.link1080);
  }
...
//  Download text
  ...
  void _showMsg(UserProfileModel userDetails) {
    if (userDetails.paypal.length == 0 || userDetails.user.subscriptions == null || userDetails.user.subscriptions.length == 0) {
  ...
  Widget column() {
    ...
    if(download == 0){
      ...
      return userProfileProvider.active == "1" ? Builder(
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      final userProfileProvider = Provider.of<UserProfileProvider>(context, listen: false).userProfileModel;

        ...
        child: AlertDialog(
          ...
          content: Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                Text("Uploading Details...", style: TextStyle(color: Theme.of(context).backgroundColor),),
...
//  Saving payment details to your server so that user details can be updated either user is subscribed or not subscribed.
  Future<String> sendPaymentNonce() async {
    ...
    if (sendNonceResponse.statusCode == 200) {
      ...
      goToDialog(createdDate, createdTime, planDetails[widget.planIndex].amount);
    } else{
      setState(() {
        isShowing = false;
      });
...
*/
  ...
  Widget closeFloatingButton() => FloatingActionButton(
    ...
    onPressed: () => Navigator.pushNamed(context, RoutePaths.splashScreen, arguments: SplashScreen(token: authToken,)),
  );
  ...
  Widget makeListTile1(){
    ...
    return ListTile(
          ...
          children: <Widget>[
            ...
            Row(
              children: <Widget>[
                Expanded(
                  flex: 1,
                  child: Text('Min duration '+ '${planDetails[widget.planIndex].intervalCount}'+' days',
      ...
      Column(
          children:<Widget>[
            Text("Amount: "+'\n' +
                '${planDetails[widget.planIndex].amount} ' + '${planDetails[widget.planIndex].currency}'),
          ]
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        child: Scaffold(
          appBar: customAppBar(context, "Braintree Payment"),
          body: _body(),

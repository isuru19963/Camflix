...
class PaypalPayment extends StatefulWidget {
  ...
  PaypalPayment({this.onFinish, this.currency, this.userFirstName, this.userLastName, this.userEmail, this.payAmount, this.planIndex});
  ...
  Widget build(BuildContext context) {
    if (checkoutUrl != null) {
      return Scaffold(
        ...
        appBar: customAppBar(context, "Paypal Payment"),
        body: WebView(
              ...
              if (payerID != null) {
                services.executePayment(executeUrl, payerID, accessToken)
                  ...
                  Navigator.push(context, MaterialPageRoute(builder: (context) => PaypalScreen(payId: ls[0], saleId: ls[1],
                    method: "Paypal", amount: widget.payAmount, planIndex: widget.planIndex)));
                });
              } else {
                Navigator.of(context).pop();
              }
              Navigator.push(context, MaterialPageRoute(builder: (context) => PaypalScreen(payId: payResponse[0],
                saleId: payResponse[1], method: "Paypal", amount: widget.payAmount, planIndex: widget.planIndex)));
              Navigator.of(context).pop();
            }

...
import 'package:nexthour/ui/widgets/credit_card_form.dart';
import 'package:nexthour/ui/widgets/credit_card_model.dart';
import 'package:nexthour/ui/widgets/credit_card_widget.dart';
import 'package:nexthour/ui/widgets/profile_tile.dart';
...
//  Customer is created on stripe for making payment.
  Future<String> _createCustomer(name, email, stripePass, planId) async {
    final menuResponse = await http.post(
        ...
        headers: {
          HttpHeaders.authorizationHeader: "Bearer $stripePass"
        });
...
//  Stripe card is automatically saved for customer for future payment.
  ...
  Future<String> _saveCardForCustomer(customerStripeId, cardid, stripePass, planId) async {
    ...
    if(saveCardResponse.statusCode == 200){
      ...
      _createSubscription(customerStripeId, cardid, cardtype, cardBrand, cardLast4, stripePass,  planId);
    }else{
...
//  Creating stripe subscription form the customer using customer Id and plan.
  Future<String> _createSubscription(customerStripeId, cardid, cardtype, cardBrand, cardLast4, stripePass,  planId) async {
    var subscriptionResponse;
    if(widget.couponCode != ''){
      var stripeUri = "https://api.stripe.com/v1/customers/"+"$customerStripeId"+"/subscriptions?plan="
          ""+"$planId"+"&quantity=1&default_source="+"$cardid"+"&coupon="+"${widget.couponCode}";
      ...
      subscriptionResponse = await http.post(
          Uri.encodeFull("https://api.stripe.com/v1/customers/"+"$customerStripeId"+"/"
              "subscriptions?plan="+"$planId"+"&quantity=1&default_source="+"$cardid"
          ),
          headers: {
            HttpHeaders.authorizationHeader: "Bearer $stripePass"
          });
...
//  Payment Process on tapping button
  Widget floatingBar() {
    ...
    var stripeKey = Provider.of<PaymentKeyProvider>(context, listen: false).paymentKeyModel.key;
    var stripePass= Provider.of<PaymentKeyProvider>(context, listen: false).paymentKeyModel.pass;
              ...
              }else
              {
                if(stripeKey == null){
                  ...
                  if(stripeCustomerId != null){
                    ...
                    _createCustomer(userDetails.user.name, userDetails.user.email, stripePass, planList[widget.index].planId);
                  }
                  showSuccessDialog();
                }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              ListTile(
                title: Text(userDetails.user.name,style: TextStyle(color: Colors.black),),
                subtitle: Text(userDetails.user.email,style: TextStyle(color: Color.fromRGBO(20, 20, 20, 1.0)),),
                trailing: userDetails.user.image != null ? Image.network(
              ...
              ListTile(
                ...
                subtitle: Text("${planList[widget.index].amount}"+" ${planList[widget.index].currency}",style: TextStyle(color: Color.fromRGBO(20, 20, 20, 1.0)),),
...
//  Container for error message
                ...
                FloatingActionButton(
                  ...
                  onPressed: () {
                    var router = new MaterialPageRoute(
                        builder: (BuildContext context) => SplashScreen(token: authToken,)
                    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _visible == false ? CircularProgressIndicator(

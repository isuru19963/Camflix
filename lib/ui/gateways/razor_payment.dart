...
class _RazorPaymentState extends State<RazorPayment> {
  ...
  Widget payButtonRow(){
    var razorPayKey = Provider.of<PaymentKeyProvider>(context).paymentKeyModel.razorkey;
    var planList = Provider.of<AppConfig>(context).planList;
    return Row(
      children: <Widget>[
        Expanded(
          flex: 1,
            ...
            child: RaisedButton(
              ...
              onPressed: (){
                if(razorPayKey == null){
                  ...
                  return;
                }else if(planList[widget.index].currency != "INR"){
                  Fluttertoast.showToast(msg: "Razorpay support only INR currency.");
                  return;
                } else{
                  openCheckout();
                }
  ...
  Widget build(BuildContext context) {
    ind=widget.index;
    return Scaffold(
      appBar: customAppBar(context, "RazorPay Payment"),
      body: _body(),
    );
  }
  ...
  void openCheckout() async {
    ...
    var razorPayKey = Provider.of<PaymentKeyProvider>(context, listen: false).paymentKeyModel.razorkey;
    double cost;
    dynamic amountdata = widget.payAmount == null ? planDetails[widget.index].amount : widget.payAmount;
          ...
          builder: (context) =>
              WillPopScope(
                  child: AlertDialog(
                    ...
                    title: Text("Saving Payment Info", style: TextStyle(color: Theme.of(context).backgroundColor),),
                    content: Container(
                      height: 70.0,
                      width: 150.0,
                      child: Center(

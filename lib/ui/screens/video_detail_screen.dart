...
import 'package:nexthour/services/download/download_episode_page.dart';
...
import 'package:nexthour/ui/shared/seasons_artist_list.dart';
import 'package:nexthour/ui/shared/share_page.dart';
import 'package:nexthour/ui/shared/tab_widget.dart';
import 'package:nexthour/ui/shared/wishlist.dart';
import 'package:nexthour/ui/widgets/seasons_tab.dart';
...
class _VideoDetailScreenState extends State<VideoDetailScreen>  with TickerProviderStateMixin, RouteAware{
            ...
            return Container(
              child: Column(
                ...
                children: [
                  ...
                  widget.videoDetail.actors.length== 0 ? SizedBox.shrink() : heading("Artist"),
                  SizedBox(
                    height: 5.0,
                  ),
  ...
  Widget moreDetails() {
    var commentsStatus = Provider.of<AppConfig>(context, listen: false).appModel.config.comments;
    ...
    return SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            ...
            commentsStatus == 1 || "$commentsStatus" == "1" ? comments() : SizedBox.shrink(),
          ],
      ),
    );
  }
          ...
          Row(
            ...
            children: [
              ...
              RaisedButton(onPressed: (){
                addComment(context);
              }, child: Text("Add"), color: activeDotColor,),
    ...
    for(int i= 0; i<comments.length; i++){
      list.add(Container(
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(comments[i].name, style: TextStyle(fontSize: 16.0, color: Colors.white.withOpacity(0.9))),
            SizedBox(
              height: 6.0,
            ),
            Text(DateFormat.yMMMd().format(DateTime.parse("${comments[i].createdAt}"),), style: TextStyle(fontSize: 12.0, color: Colors.white.withOpacity(0.6))),
            SizedBox(
              height: 10.0,
            ),
            Text(comments[i].comment, style: TextStyle(fontSize: 14.0, color: Colors.white.withOpacity(0.7))),
            SizedBox(
              height: 15.0,
            ),
          ],
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: Container(
            ...
            child: Text('Add Comments',style: TextStyle(color: activeDotColor, fontWeight: FontWeight.w600),),
          ),
          content: Container(
  ...
  Future <String> postComment() async {
    ...
    if(postCommentResponse.statusCode == 200){
      commentList.add(Comment(id: widget.videoDetail.id, name: "${userDetails.user.name}", email: "${userDetails.user.email}",
          comment: commentsController.text,
  ...
  Widget audioLangRow(videoDetail){
    if(widget.videoDetail.audios != null || "${widget.videoDetail.audios}" != "null"){
  ...
  Widget cusPlaceHolder(moreLikeThis, index){
    return InkWell(
      child: moreLikeThis[index].thumbnail == null ? Image.asset('assets/placeholder_box.jpg',
        height: 150.0,
        fit: BoxFit.cover,
      ...
      onTap: () {
        Navigator.pushNamed(context, RoutePaths.videoDetail, arguments: VideoDetailScreen(moreLikeThis[index]));
      },
    );
  }
...
//   Getting list of episodes of different seasons
  Future<String> getData(currentIndex) async {
    ...
    final episodesResponse = await http.get(
        Uri.encodeFull(
            APIData.episodeDataApi + "${widget.videoDetail.seasons[currentIndex].id}"),
        headers: {
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            seasonEpisodeData  == null ? SizedBox.shrink() : widget.videoDetail.seasons[0].actorList.length == 0 ? SizedBox.shrink() :
            heading("Artist"),
            SizedBox(
              height: 10.0,
            ),
...
*/
                  ...
                  return new Padding(
                    ...
                    child: moreLikeThis[index] == null
                      ...
                      child: moreLikeThis[index].thumbnail == null ? Image.asset('assets/placeholder_box.jpg',
                        ...
                        image: "${APIData.tvImageUriTv}${moreLikeThis[index].thumbnail}",
                        placeholder: 'assets/placeholder_box.jpg',
                        height: 150.0,
                        fit: BoxFit.cover,
                      ),
                      onTap: () => Navigator.pushNamed(context, RoutePaths.videoDetail, arguments: VideoDetailScreen(moreLikeThis[index])),
                    ),
                  );
                }),
          ),
  ...
  Widget gestureDetector(i) {
    var userDetails = Provider.of<UserProfileProvider>(context).userProfileModel;
  ...
  void initState() {
   ...
   setState(() {
    ...
    _seasonsTabController = TabController(vsync: this,
        length: widget.videoDetail.seasons == null ? 0 : widget.videoDetail.seasons.length,
        initialIndex: 0);

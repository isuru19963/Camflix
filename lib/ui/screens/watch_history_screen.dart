...
import 'package:nexthour/ui/widgets/blank_watch_history.dart';
...
Future<String> removeHistory(vType, id) async{
  ...
  final response  = await http.get("${APIData.deleteWatchHistory}$type/$id", headers: {
    HttpHeaders.authorizationHeader: "Bearer $authToken"
  });
  if(response.statusCode == 200){
    watchHistoryList.removeWhere((element) => element.type == vType && element.id == id);
    setState(() {
    });
  }
  return null;
  ...
  Future<String> removeAllHistory() async{
    final response  = await http.get("${APIData.deleteWatchHistory}", headers: {
      HttpHeaders.authorizationHeader: "Bearer $authToken"
    });
    if(response.statusCode == 200){
  ...
  getWatchHistory() async{
    ...
    final watchHistory = Provider.of<WatchHistoryProvider>(context, listen: false).watchHistoryModel.watchHistory;
        ...
        if(moviesTvList[i].type == DatumType.T){
          if(moviesTvList[i].id == watchHistory[j].tvId){
            ...
            watchHistoryList.add(Datum(
              ...
              seasons: List.generate(moviesTvList[i].seasons == null ? 0 : moviesTvList[i].seasons.length, (sIndex) {
                var actors = moviesTvList[i].seasons[sIndex].actorId == ""  || moviesTvList[i].seasons[sIndex].actorId == null ? null
                    : moviesTvList[i].seasons[sIndex].actorId.split(",").toList();
                return Season(
                  ...
                  episodes: List.generate(
                      moviesTvList[i].seasons[sIndex].episodes == null ? 0 : moviesTvList[i].seasons[sIndex].episodes.length,
                          (eIndex) {
                        return Episode(
...
Widget build(BuildContext context) {
  return Scaffold(
    ...
    body: _visible == false ? Center(child: CircularProgressIndicator(),) : watchHistoryList.length == 0 ? BlankWatchHistory() : ListView.builder(
        itemCount: watchHistoryList.length,
        padding: EdgeInsets.only(left: 15.0,right: 15.0,top: 5.0, bottom: 10.0),
        itemBuilder: (context,index){
           watchHistoryList[index].genres.removeWhere((element) => element == null);
           String s = "${watchHistoryList[index].genres}".replaceAll("[", "");
                  ...
                  child: ClipRRect(
                      ...
                      child: Material(
                          ...
                          child: InkWell(
                                  ...
                                  children: [
                                    ClipRRect(
                                      ...
                                      child: FadeInImage.assetNetwork(
                                          ...
                                          image: watchHistoryList[index].type == DatumType.M ?
                                    ...
                                    Expanded(child: Container(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text('${watchHistoryList[index].title}',
                                          ...
                                          watchHistoryList[index].duration == null ? SizedBox.shrink() :
                                          Text('Duration : ${watchHistoryList[index].duration} min',
                                              textAlign: TextAlign.start,
                                              style: TextStyle(
                                                  color:
                                          ...
                                          Row(
                                            ...
                                            children: [
                                              RatingBar.builder(
                                                initialRating: double.parse("${watchHistoryList[index].rating / 2}"),
                                                minRating: 1,
                                              ...
                                              _selectPopup(watchHistoryList[index]),
                                            ],
                                          ),
                                        ],
                                      ),
                            ...
                            onTap: () {
                              Navigator.pushNamed(context, RoutePaths.videoDetail, arguments: VideoDetailScreen(watchHistoryList[index]));
                            },
                          ))),
                ),
                SizedBox(height: 10),

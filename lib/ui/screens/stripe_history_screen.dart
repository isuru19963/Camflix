...
class _StripeHistoryScreenState extends State<StripeHistoryScreen> {
  ...
  List<Subscription> _stripeHistoryList() {
    var stripePayment = Provider.of<UserProfileProvider>(context, listen: false).userProfileModel.user.subscriptions;
    return List<Subscription>.generate(stripePayment == null ? 0 : stripePayment.length, (int index){
      return Subscription(
                          ...
                          Expanded(
                            ...
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                subscriptionFromTo(DateFormat("d-m-y HH:MM").format(stripePayment[j].subscriptionFrom),
                                    DateFormat("d-m-y HH:MM").format(stripePayment[j].subscriptionTo)),
                              ],
                            ),
                          ),
                        ],
  ...
  Widget build(BuildContext context) {
    var stripePayment = Provider.of<UserProfileProvider>(context, listen: false).userProfileModel.user.subscriptions;
    return SafeArea(
      child: stripePayment != null ? Scaffold(
          appBar: customAppBar(context, "Stripe Payment History"),

...
class _ApplyCouponScreenState extends State<ApplyCouponScreen> {
  ...
  Future<String> _verifyCoupon(couponCode) async {
    var couponProvider  = Provider.of<CouponProvider>(context, listen: false).couponModel;
    final applyCouponResponse = await http.post( APIData.applyGeneralCoupon, headers: {
          HttpHeaders.authorizationHeader: "Bearer $authToken"
        },
      body: {
    ...
    if (applyCouponResponse.statusCode == 200) {
      for(int i=0; i<couponProvider.coupon.length; i++){
        if(couponProvider.coupon[i].couponCode == couponCode){
          if(couponProvider.coupon[i].inStripe == "1" || couponProvider.coupon[i].inStripe == 1){
            _applyCoupon();
          }else{
            mFlag = 1;
            setState(() {
      ...
      list.add(
          Container(
            ...
            child: Column(
              ...
              children: [
                cList[i].percentOff == null ? Text("${cList[i].couponCode} ${cList[i].amountOff}"
                    "${cList[i].currency} off", style: TextStyle(fontSize: 18.0),)
                    : Text("${cList[i].couponCode} ${cList[i].percentOff}"
          " ${cList[i].currency} % off", style: TextStyle(fontSize: 18.0),)
              ],
            ),
          )
                    ...
                    children: [
                      Expanded(flex: 3, child: Container(
                        ...
                        child: TextFormField(
                          ...
                          onSaved: (val) => _couponController.text = val,
                        ),
                      ),),

...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Widget notificationIconContainer(){
    return Container(
      child: Icon(Icons.notifications, size: 170.0, color: Color.fromRGBO(70, 70, 70, 1.0),),
    );
  }
  ...
  Widget message(){
    return  Padding(padding: EdgeInsets.only(left: 50.0, right: 50.0),
      child: Text("You don't have any notification.",
        style: TextStyle(height: 1.5,color: Colors.white70.withOpacity(0.5), fontSize: 18.0),
      ),
    );
  }
...
//  When don't have any notification.
  ...
  var notifications = Provider.of<NotificationsProvider>(context).notificationsList;
    return Scaffold(
      appBar: customAppBar(context, "Notifications"),
      body: _visible == false ? Center(child: CircularProgressIndicator(),) : notifications.length == 0 ?
      blankNotification() : ListView.builder(
        ...
        return Column(
          children: <Widget>[
            ListTile(
              ...
              subtitle: Text("${notifications[index].data.data}",
                style: TextStyle(
                fontSize: 12.0,
              ),
                maxLines: 1,
              ...
              onTap: (){
                Navigator.pushNamed(context, RoutePaths.notificationDetail,
                    arguments: NotificationDetailScreen(notifications[index].title, notifications[index].data.data));
              },
            ),
            SizedBox(
              height: 5.0,

...
class _ActorMoviesGridState extends State<ActorMoviesGrid> {
  ...
  Widget build(BuildContext context) {
    var actorMoviesList = Provider.of<ActorMoviesProvider>(context).actorMoviesTVList;
    return Scaffold(
      appBar: customAppBar(context, "${widget.actorDetails.name}"),
      body: _visible == false ? Center(child: CircularProgressIndicator(),) : actorMoviesList.length == 0 ? NoMovies() : Container(
                  ...
                  Container(
                    ...
                    child: Material(
                      ...
                      child: InkWell(
                        ...
                        child: ClipRRect(
                          borderRadius: new BorderRadius.circular(10.0),
                          child: actorMoviesList[index].thumbnail == null ?
                          Image.asset("assets/placeholder_box.jpg",
                            fit: BoxFit.cover,
                          ) :
                          FadeInImage.assetNetwork(
                            image: actorMoviesList[index].type == DatumType.M ?
                        ...
                        onTap: () {
                          Navigator.pushNamed(context, RoutePaths.videoDetail, arguments: VideoDetailScreen(actorMoviesList[index]));
                        },
                      ),
                    ),
                  ),

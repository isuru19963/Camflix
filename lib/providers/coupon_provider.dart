  ...
  Future<CouponModel> getCoupons(BuildContext context) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        stripeCoupon = List<Coupon>.generate(couponModel.coupon.length, (index) {
          if(couponModel.coupon[index].inStripe == 1 || "${couponModel.coupon[index].inStripe}" == "1"){
            return Coupon(
        ...
        generalCoupon = List<Coupon>.generate(couponModel.coupon.length, (index) {
          if(couponModel.coupon[index].inStripe == 0 || "${couponModel.coupon[index].inStripe}" == "0"){
            return Coupon(
              id: couponModel.coupon[index].id,
              couponCode: couponModel.coupon[index].couponCode,
              percentOff: couponModel.coupon[index].percentOff,

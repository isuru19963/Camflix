...
class _PlayerMovieState extends State<PlayerMovie> with WidgetsBindingObserver {
  ...
  Future<String> loadLocal() async {
    ...
    playerResponse = await http.get(widget.type == DatumType.T
        ? APIData.tvSeriesPlayer + '${userDetails.user.id}/${userDetails.code}/$ser'
        : APIData.moviePlayer + '${userDetails.user.id}/${userDetails.code}/${widget.id}');
    setState(() {
  ...
  Widget build(BuildContext context) {
    var userDetails = Provider.of<UserProfileProvider>(context, listen: false).userProfileModel;
    print(widget.type == DatumType.T ? APIData.tvSeriesPlayer + '${userDetails.user.id}/${userDetails.code}/$ser' :
    APIData.moviePlayer + '${userDetails.user.id}/${userDetails.code}/${widget.id}');
    SystemChrome.setPreferredOrientations(

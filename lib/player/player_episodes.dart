...
//        Navigator.pop(context);
  ...
  Widget build(BuildContext context) {
    var userDetails = Provider.of<UserProfileProvider>(context, listen: false).userProfileModel;
    print("Episode URL: ${APIData.episodePlayer+'${userDetails.user.id}/${userDetails.code}/${widget.id}'}");
          ...
          body: Stack(
            children: <Widget>[
              Container(
                ...
                height: height,
                child: WebView(
                  initialUrl: APIData.episodePlayer+'${userDetails.user.id}/${userDetails.code}/${widget.id}',
...
class LocalLoader{
  Future<String> loadLocal(BuildContext context) async {
    var userDetails = Provider.of<UserProfileProvider>(context, listen: false).userProfileModel;
    return await rootBundle.loadString(APIData.moviePlayer+'${userDetails.user.id}/${userDetails.code}/$x');
  }
}